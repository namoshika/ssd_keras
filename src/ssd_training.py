...
class MultiboxLoss(object):
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        # loss for all priors
        conf_loss = self._softmax_loss(y_true[:, :, 4:-1], y_pred[:, :, 4:-1])
        ...
        full_indices = (tf.reshape(batch_idx, [-1]) * tf.cast(num_boxes, dtype=tf.int32) + tf.reshape(indices, [-1]))
...
class Generator(object):
    ...
    def generate(self, train=True):
        while True:
            if train:
                rnd.shuffle(self.train_keys)
                keys = self.train_keys
            else:
                rnd.shuffle(self.val_keys)
                keys = self.val_keys
            inputs = []
            targets = []
            for key in keys:            
